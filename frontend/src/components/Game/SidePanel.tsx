...
export default function SidePanel({ timerStr, log, onAbandon }: Props) {
  ...
  return (
    <aside className="w-52 flex-shrink-0 border-l border-sky-400/10 bg-[rgba(3,15,30,0.5)] backdrop-blur-xl p-5 flex flex-col gap-5 overflow-y-auto">
      ...
      {/* Log */}
      <div className="flex flex-col flex-1 min-h-0 animate-[fadeInUp_.6s_ease_.4s_forwards] opacity-0">
        <PanelTitle>Registre</PanelTitle>
        <div className="flex flex-col gap-1 overflow-y-auto">
          {log.map((e) => (
            <div
              key={e.id}
              className={`flex items-center gap-2 px-2.5 py-2 rounded-xl border font-[Cinzel] text-[.6rem] tracking-[.05em] fade-up transition-all ${
                e.type === "found"
      ...
      {/* Abandon button */}
      <div className="animate-[fadeInUp_.6s_ease_.5s_forwards] opacity-0">
        <button
          onClick={onAbandon}
